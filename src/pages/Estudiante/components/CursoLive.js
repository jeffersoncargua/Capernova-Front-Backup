...
export const CursoLive = ({estudiante}) => {
  ...
  return (
    <div>
        <h1 className="text-center font-medium text-xl dark:text-white mb-10">Cursos en Vivo de Capernova</h1>
                    ...
                    {matriculaList.length > 0 && matriculaList.map((matricula) => (
                        <tr key={matricula.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 line-clamp-2 ">
                                {matricula.curso.claseUrl ? 'Cuenta con clases en vivo':'No cuenta con clases en vivo'}
                            </td>
                            <td className="px-4 py-3">
                                {matricula.curso.claseUrl ? 
                                (<div className="py-1 flex justify-start" >
                                    <Link to={matricula.curso.claseUrl} target="_blank" className="flex items-center justify-center py-2 px-4 text-sm text-black dark:text-white bg-green-500 hover:bg-green-600 rounded-lg mr-2">
                                         <svg xmlns="http://www.w3.org/2000/svg"fill="currentColor" class="bi bi-check-circle w-5 h-5 me-3 text-black dark:text-white" viewBox="0 0 16 16">

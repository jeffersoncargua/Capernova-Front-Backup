...
export const ProgressBar = ({matricula,setShowModalDownload, setResult }) => {
  ...
  const handleGetCertificate = async() => {
    setLoading(true);
    try {
      const result = await fetch(`https://localhost:7164/api/Student/getCertificate?studentId=${matricula.estudianteId}&cursoId=${matricula.cursoId}`,{
        method: 'GET',
        headers:{
  ...
  return (
    ...
    <div>
      {showLoading && <Loading /> }
      <button onClick={() => handleGetCertificate()} disabled={(matricula.estado === 'Completado')? false:true} className={`${(matricula.estado === 'Completado')? 'hover:cursor-pointer':'hover:cursor-not-allowed'}`}>
        ...
        (<div data-tooltip-id='tooltip-certificado' className=' mb-1'>
          ...
          <svg className="" viewBox="0 0 100 100">
            ...
            <circle
              ...
              strokeDashoffset={`calc(282.74 - (282.74 * ${matricula.estado === 'Completado' ? 100:0} ) / 100)`} // permite rellenar la circunferencia de lo que falta del avance
            ></circle>
            
            {/*<!-- Center text -->*/}
            <svg x='30' y='30' xmlns="http://www.w3.org/2000/svg" fill="currentColor" className={`bi bi-trophy-fill z-10 w-1 h-1 ${matricula.estado === 'Completado' ? 'text-yellow-300':'text-gray-300'}`} viewBox="0 0 40 40">

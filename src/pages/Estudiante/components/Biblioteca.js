...
export const Biblioteca = ({estudiante}) => {
  ...
  return (
    <div>
        <h1 className="text-center font-medium text-xl dark:text-white mb-10">Biblioteca Capernova</h1>
                    ...
                    {matriculaList.length > 0 && matriculaList.map((matricula) => (
                        <tr key={matricula.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4 line-clamp-2 ">
                                {matricula.curso.bibliotecaUrl ? 'Con Acceso a Biblioteca':'Sin Acceso a Biblioteca'}
                            </td>
                            <td className="px-4 py-3">
                                {matricula.curso.bibliotecaUrl ? 
                                (<div className="py-1 flex justify-start" >
                                    <Link to={matricula.curso.bibliotecaUrl} target="_blank" className="flex items-center justify-center py-2 px-4 text-sm text-black dark:text-white bg-green-500 hover:bg-green-600 rounded-lg mr-2">

		...
		try {
			...
			if (refImageUrl.current !== null && refImageUrl.current !== undefined) {
				const result = await fetch(
					`${process.env.REACT_APP_API_URL}/Student/updateImageStudent?id=${estudiante.id}`,
					{
						method: "PUT",
						credentials: "include",
						headers: {
					...
					<div className="flex justify-center w-60 h-60 relative rounded-full outline outline-4 outline-gray-300 shadow-lg bg-slate-50 z-20">
						{estudiante.photoUrl ? (
							<img
								...
								src={`https://drive.google.com/thumbnail?id=${estudiante.photoUrl}`}
								alt="Aqui va la foto"
							/>
						) : (
							<img
						...
						<Tooltip
							...
							place="top"
							content={
								uploadFile ? "No Cambiar Foto Perfil" : "Cambiar Foto Perfil"
							}
						/>
					</div>

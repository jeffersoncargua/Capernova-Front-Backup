		...
		<div className=" relative w-full flex flex-col mb-10 md:shadow-2xl md:shadow-slate-50 md:border-b md:border-slate-50 ">
			...
			<div className="flex-1 " data-aos="fade-up">
				...
				<video
					...
					src={video}
					className="flex object-cover w-full h-auto md:h-[30rem]"
					autoPlay
					loop
					muted
					controls={false}
			...
			<div className="w-full flex flex-col items-center md:flex-row bg-gradient-to-r from-teal-400 to-blue-500 hover:from-pink-500 hover:to-orange-500">
				<div
					className="w-full order-2 md:order-1 md:w-1/2 overflow-x-hidden relative z-20 "
					data-aos="fade-up"
				>
					<div
						className="flex transition ease-in-out duration-1000 z-40"
					...
					>
						{slices.map((s, index) => (
							...
							>
								<h1
									translate="no"
									className={` font-kaushanScript text-white font-extrabold text-6xl m-[5%] sm:text-7xl md:text-[5rem] min-[807px]:text-[5.5rem] lg:text-[6.5rem] min-[1250px]:text-[8.5rem] text-center ${current === index ? "animate-in zoom-in-50 duration-4000 " : ""}`}
									style={{
										textShadow:
											"5px 5px 15px black,1px 1px 2px black,15px 15px 30px black",
									}}
								>
									{s.titulo}
								</h1>
				...
				<div
					className="bg-slate-50 overflow-x-hidden w-full order-1 md:order-2 md:w-1/2 z-10 md:shadow-2xl md:shadow-slate-50 md:border-slate-50"
					data-aos="fade-up"
				>
					<div
					...
					>
						{slices.map((s, index) => (
							<div className="shrink-0 w-full" key={Math.random()}>
								<img
									src={`https://lh3.googleusercontent.com/d/${s.imageUrl}`}
									alt="slice"
									className={`w-full opacity-100 ${current === index ? "animate-in zoom-in-90 duration-4000 " : "animate-out zoom-out duration-1000 "} `}
								/>
							</div>
						))}
					</div>

...
export const Feedback = () => {
	...
	return (
		<div
			className="w-[95%] flex flex-col my-[50px] mx-auto dark:bg-gray-900"
			data-aos="fade-up"
		>
			...
			<div className="overflow-hidden w-full dark:border-gray-700 md:mb-12 bg-white dark:bg-gray-900 my-8">
				...
				<div
					className="md:flex hidden bg-white w-[98%] transition ease-in-out duration-400 gap-x-2 dark:bg-gray-900"
					style={{ transform: `translateX(-${feedback * 50}%)` }}
				>
					{users.map((user) => (
						<figure
							key={Math.random()}
							className="flex-none flex flex-col items-center justify-center border border-gray-200 rounded-lg shadow-sm p-8 text-center bg-white dark:bg-gray-800  w-full md:w-1/2"
						>
							<blockquote className="max-w-2xl mx-auto mb-4 text-gray-500 lg:mb-8 dark:text-gray-400">
								<h3 className="text-lg font-semibold text-gray-900 dark:text-white">
									Curso de {user.titulo}
								</h3>
							...
							<figcaption className="flex items-center justify-center ">
								<img
									...
									src={
										`https://drive.google.com/thumbnail?id=${user.photoUrl}` ||
										Avatar
									}
									alt="profilepicture"
								/>
				...
				<div
					className="flex md:hidden bg-white w-[98%] transition ease-in-out duration-400 gap-x-2 dark:bg-gray-900"
					style={{ transform: `translateX(-${feedback * 100}%)` }}
				>
					{users.map((user) => (
						<figure
							key={Math.random()}

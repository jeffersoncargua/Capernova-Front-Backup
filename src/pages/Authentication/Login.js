...
export const Login = ({ children }) => {
	...
	const handleSubmitLogin = async (event) => {
		...
		try {
			const resultFromApi = await LoginPost({
		...
		<div className="w-[95%] md:flex mx-auto">
			...
			<div className="md:w-1/2 w-full my-10 border border-gray-300 p-6 flex items-center">
				<form className="w-[70%] mx-auto" onSubmit={handleSubmitLogin}>
					...
					<Link
						to="/forget"
						className="text-blue-500 text-sm my-10 font-bold hover:underline hover:decoration-blue-500 block"
					>
			...
			{showModal && (
				<ModalLogin response={response} setShowModal={setShowModal} />
			)}
			{children}
		</div>
	);

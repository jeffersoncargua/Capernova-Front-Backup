...
export const SearchFilter = () => {
  ...
  return (
    <div className='relative'>
        ...
        <div className={`group text-black dark:text-white absolute top-0 left-0 z-40 w-[95%] min-[350px]:w-56 flex flex-wrap space-x-4 justify-start ps-4 border border-gray-200 rounded-lg dark:border-gray-700 gap-y-8 ml-4 px-10 py-2 transition-transform ${showSearch ? '-translate-x-full':''} md:translate-x-0 bg-slate-50 dark:bg-gray-900 drop-shadow-md `} tabIndex="-1">
            <h1 className="text-xl md:text-2xl font-semibold text-center md:text-start">Búsqueda por categoría</h1>
            ...
            <div key={categoria.id} className="mb-2">  
                <Link to={`/products?categoriaId=${categoria.id}`} className="bg-gray-300 text-sm md:text-base dark:bg-gray-700 px-2.5 py-2" >{categoria.name}</Link>
            </div>)))
            :
            (<div className="">                  
            ...
            <div className="mb-2">  
                <Link onClick={()=>dispatch(remove())} to={`/products`} className="bg-gray-300 dark:bg-gray-700 text-sm md:text-base px-2.5 py-2" >Ver Todos</Link>
            </div>

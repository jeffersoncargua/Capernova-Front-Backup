...
export const SearchFilter = () => {
  ...
  return (
    <div className='w-[20%] relative md:mr-16'>
        ...
        <div className={`group text-black dark:text-white absolute top-0 left-0 z-40 flex flex-wrap space-x-4 justify-start ps-4 border border-gray-200 rounded-lg dark:border-gray-700 gap-y-8 ml-4 px-10 py-2 transition-transform ${showSearch ? '-translate-x-full':''} md:translate-x-0 bg-slate-50 dark:bg-gray-900 drop-shadow-md `} tabIndex="-1">
            <h1 className="text-2xl font-semibold text-start">Búsqueda por categoría</h1>
            ...
            <div key={categoria.id} className="mb-2">  
                <Link to={`/products?categoriaId=${categoria.id}`} className="bg-gray-300 dark:bg-gray-700 px-2.5 py-2" >{categoria.name}</Link>
            </div>)))
            :
            (<div className="">                  
                <label className="w-full py-4 ms-2 text-sm ">No existen áreas de búsqueda ... </label>
            </div> ) }

            <div className="mb-2">  
                <Link onClick={()=>dispatch(remove())} to={`/products`} className="bg-gray-300 dark:bg-gray-700 px-2.5 py-2" >Ver Todos</Link>
            </div>

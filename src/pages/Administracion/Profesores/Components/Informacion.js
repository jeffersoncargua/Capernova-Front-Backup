		...
		try {
			...
			if (refImageUrl.current !== undefined) {
				const result = await fetch(
					`${process.env.REACT_APP_API_URL}/Teacher/updateImageTeacher?id=${profesor.id}`,
					{
						method: "PUT",
						credentials: "include",
						headers: {
			...
			>
				<div className="w-[95%] mx-auto flex items-center justify-center flex-col">
					<div className="flex justify-center w-60 h-60 relative rounded-full outline outline-4 outline-gray-300 shadow-lg bg-slate-50 z-20">
						{profesor.photoURL ? (
							<img
								className="w-60 h-60 rounded-full self-center"
								src={`https://drive.google.com/thumbnail?id=${profesor.photoURL}`}
								alt="Aqui va la foto"
							/>
						) : (
							<img
						...
						<button
							...
							type="button"
							className="absolute bottom-4 right-0 w-10 h-10 bg-gray-100 group hover:bg-gray-300 dark:hover:bg-gray-400 flex justify-center items-center rounded-full border shadow"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								fill="currentColor"
								className="bi bi-camera text-slate-300 group-hover:text-black h-7 w-7"
								viewBox="0 0 16 16"
							>
						...
						<Tooltip
							...
							place="top"
							content={
								uploadFile ? "No Editar Fotografía" : "Editar Fotografía"
							}
						/>
					</div>
						...
						<div className="p-6 mt-24 flex flex-col gap-y-3">
							<div className="relative group dark:text-white">
								...
								<button
									...
									type="button"
									className="absolute bottom-2 right-2 w-8 h-8 bg-gray-100 group-hover:bg-gray-300 dark:group-hover:bg-gray-400 flex justify-center items-center rounded-lg border shadow"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										fill="currentColor"
										className="bi bi-pencil-fill text-slate-300 group-hover:text-black  h-4 w-4"
										viewBox="0 0 16 16"
									>
								...
								<Tooltip
									id="tooltip-name"
									place="top"
									content={enableName ? "No Editar Nombre" : "Editar Nombre"}
								/>
							</div>
							<div className="relative group dark:text-white">
								...
								<button
									...
									data-tooltip-id="tooltip-lastName"
									type="button"
									className="absolute bottom-2 right-2 w-8 h-8 bg-gray-100 group-hover:bg-gray-300 dark:group-hover:bg-gray-400 flex justify-center items-center rounded-lg border shadow"
								>
									<svg
								...
								<Tooltip
									...
									place="top"
									content={
										enableLastName ? "No Editar Apellido" : "Editar Apellido"
									}
								/>
							</div>
							<div className="relative group dark:text-white">
								...
								<Tooltip
									...
									place="top"
									content={
										enablePhone ? "No Editar Teléfono" : "Editar Teléfono"
									}
								/>
							</div>
							<div className="w-[95%] relative group dark:text-white">
								...
								<textarea
									className={`w-full ${!enableBiography ? "border-0 bg-transparent" : "border-1 border-blue-300 bg-blue-200 dark:bg-slate-900"} rounded-lg px-0 py-3 text-justify`}
									rows="8"
									required
								...
								></textarea>
								<button
									onClick={() => setEnableBiography(!enableBiography)}
									data-tooltip-id="tooltip-biography"
									type="button"
									className="absolute bottom-2 right-2 w-8 h-8 bg-gray-100 group-hover:bg-gray-300 dark:group-hover:bg-gray-400 flex justify-center items-center rounded-lg border shadow"
								>
									<svg
								...
								<Tooltip
									id="tooltip-biography"
									place="top"
									content={
										enableBiography ? "No Editar Biografía" : "Editar Biografía"
									}
								/>
							</div>
						</div>
				...
				<div>
					{uploadFile && (
						<input
							...
							className="ms-10 mt-10 rounded-full border border-blue-300"
							required
							ref={refImageUrl}
						/>
					)}

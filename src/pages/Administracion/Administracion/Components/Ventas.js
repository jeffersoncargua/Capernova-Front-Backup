...
export const Ventas = () => {
  ...
  useEffect(() => {
    const fetchVentas = async() => {
      const resultFromApi = await fetch(`https://localhost:7164/api/Venta/getAllVentas?search=${search}`,{
        method:'GET',
        credentials : 'include',
        headers:{
  ...
  }, [showModal,showModalDelete,search,response]);
  ...
  return (
    <div>
      ...
      {/* {showModal && <Modalventa showModal={showModal} setShowModal={setShowModal} venta={venta} setResponse={setResponse} />} */}
              ...
              <table className="w-full text-sm text-left dark:text-white">
                ...
                {ventaList.length > 0 && ventaList.map((item) => (
                  <tr key={item.id} className="border-b dark:border-gray-700">
                    <td className="px-4 py-3">{item.emision}</td>

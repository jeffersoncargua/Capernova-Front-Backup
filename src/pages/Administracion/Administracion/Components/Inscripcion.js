...
export const Inscripcion = () => {
    ...
    useEffect(()=>{
            ...
            const fetchMatricula = async() => {
                if (user.id) {
                    const resultFetchMatricula = await fetch(`${process.env.REACT_APP_API_URL}/Managment/getMatricula?userId=${user.id}`,{
                        method: "GET",
                        credentials: "include",
                        headers: {
    ...
    const handleSearchUser = async(e) => {
        e.preventDefault();
        try {
            const resultFromApi = await fetch(`${process.env.REACT_APP_API_URL}/Managment/getUser?search=${refUser.current.value}`,{
                method : "GET",
                credentials:"include",
                headers:{
                    "Content-Type" : "application/json",
            ...
            toast.error("Error. Algo ocurrió en nuestros servidores. Inténtelo nuevamente!!!");
        }
    }
  ...
  return (
    <div className="">
        ...
        {showModalInscripcion && <ModalInscripcion showModalInscripcion={showModalInscripcion} setShowModalInscripcion={setShowModalInscripcion} setResponse={setResponse} user={user} /> }
        ...
        <h1 className="text-center font-medium text-xl dark:text-white mb-10">Inscripciones Capernova</h1>
        <div className="w-[95%] mx-auto">
            ...
            { showMessage && (<h1 className={`${responseSearch.isSuccess ? 'text-green-400':'text-red-400'} mt-6 flex items-center`}><span className={`${responseSearch.isSuccess ? 'bg-green-400':'bg-red-400'} flex w-3 h-3 me-3 rounded-full`}></span> {responseSearch.isSuccess ? 'Usuario registrado':'No existe registros del usuario'}</h1>)}
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-black dark:text-white">
                        ...
                        <tbody>
                            {matriculaList.length > 0 ? (matriculaList.map((matricula) => (
                                <tr key={matricula.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td className="px-6 py-4">
                                        <button onClick={() => handleDelete(matricula)} className="flex items-center justify-center py-2 px-4 text-sm text-gray-900 hover:text-white bg-red-500 hover:bg-red-600 rounded-lg">

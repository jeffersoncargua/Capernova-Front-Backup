...
export const Productos = ({setShowProductos}) => {
    ...
    }, [currentPage,productList,numberOfPages]);*/
  ...
  return (
    <div>
      ...
      <h1 className="text-center font-medium text-xl dark:text-white mb-10">Productos de Capernova</h1>
      {/*Modal para ingresar los valores de la publicidad */}
      
      {showModal && <ModalProducto showModal={showModal} setShowModal={setShowModal} producto={producto} setResponse={setResponse} />}
          ...
          <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
              ...
              <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                  <button onClick={() => {setShowModal(!showModal);setProducto({});setResponse({})}} type="button" className="flex items-center justify-center text-gray-900 hover:text-white bg-green-500 hover:bg-green-700 focus:ring-4 focus:ring-primary-300 rounded-lg text-sm px-4 py-2 focus:outline-none dark:focus:ring-primary-800">
                ...
                {currentDataDisplayed.length > 0 ? (currentDataDisplayed.map((item) => (
                  <tr  key={item.id} className="border-b dark:border-gray-700">
                    <td className="px-4 py-3">
                      <img src={`https://lh3.googleusercontent.com/d/${item.imagenUrl}`} className="w-16 md:w-32 max-w-full max-h-full" alt="imagen producto" />
                    </td>
      ...
      <div className="flex justify-around items-center p-3 sm:p-5">
        <div className="group dark:text-white">
          <p>
            ...
            de <span>{productList?.length}</span>{" "}
            resultados
          </p>
        </div>

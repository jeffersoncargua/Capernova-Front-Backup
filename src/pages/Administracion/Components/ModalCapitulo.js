...
export const ModalCapitulo = ({
	showModalCapitulo,
	setShowModalCapitulo,
	capitulo,
	setCapitulo,
	curso,
	...
	const handleSubmitAdd = async (event) => {
		...
		try {
			...
			// 	`${process.env.REACT_APP_API_URL}/Capitulo/createCapitulo`,
			// 	{
			// 		method: "POST",
			// 		credentials: "include",
			// 		headers: {
			...
			var resultFromApi = await CreateCapitulo({
				titulo: refTitulo.current.value,
				courseId: curso.id,
			});
	...
	const handleSubmitEdit = async (event) => {
		...
		try {
			// const resultFromApi = await fetch(
			// 	`${process.env.REACT_APP_API_URL}/Capitulo/updateCapitulo/${capitulo.id}`,
			// 	{
			// 		method: "PUT",
			// 		credentials: "include",
			// 		headers: {
			...
			var resultFromApi = await UpdateCapitulo({
				id: capitulo.id,
				titulo: refTitulo.current.value,
				courseId: curso.id,
			});
	...
	return (
		<div>
			...
			<div
				id="crud-modal"
				tabIndex="-1"
				className={`${showModalCapitulo ? "" : "hidden"} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] h-full bg-gray-700/[0.6]`}
			>
				<div className="relative p-4 mx-auto w-full max-w-md max-h-full">
					...
					<div className="relative bg-white my-[10%] rounded-lg shadow dark:bg-gray-700 mb-14">
						...
						<div className="flex items-center justify-between p-4 md:p-5 dark:border-gray-600">
							...
							<button
								onClick={() => setShowModalCapitulo(false)}
								type="button"
						...
						<form
							...
							onSubmit={capitulo.id ? handleSubmitEdit : handleSubmitAdd}
						>

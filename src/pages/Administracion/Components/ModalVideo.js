...
export const ModalVideo = ({showModalVideo,setShowModalVideo,video,setVideo,videos,setVideos,capitulo,setCapitulo,setResponse/*,capitulos,setCapitulos*/}) => {
  ...
  return (
    <div>
        {/*<!-- Main modal -->*/}
        <div id="crud-modal" tabIndex='-1' className={`${showModalVideo? '' :'hidden'} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] h-full bg-gray-700/[0.6]`}>
                    ...
                    <form className="p-4 md:p-5" onSubmit={video.id ?  handleSubmitEdit : handleSubmitAdd} >
                        <div className="grid gap-4 mb-4 grid-cols-2">
                            ...
                            <div className="col-span-2">
                                ...
                                <input type="number" min={0} name="ordenReproduccion" id="ordenReproduccion" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Escribe el orden del video" required ref={refOrden} defaultValue={video.ordenReproduccion || ''} />
                            </div>
                        </div>
                        {showButtonLoading ? (<button disabled className="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">

...
export const ShoppingCart = ({ cartList, total }) => {
	...
	const handleRemoveToCart = (item) => {
		...
		if (shoppingCart.length > 0) {
			...
			const cartList = updateCartList.filter(
				(itemCart) => itemCart.productoId !== String(item.id),
			);
	...
	return (
		<div className="w-[95%] mx-auto flex flex-col gap-y-8" data-aos="fade-up">
			...
			<h1 className="font-medium text-xl text-center dark:text-white">
				Tu carrito contine:{" "}
				<span className="text-pink-400 text-2xl dark:text-pink-500">
					{cartList.length}
				</span>{" "}
				{cartList.length > 1 ? "productos" : "producto"}
			</h1>
					...
					<tbody>
						{cartList.length > 0 &&
							cartList.map((item) => (
								<tr
									key={item.id}
									className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-green-100 dark:hover:bg-gray-600"
								>
									<td className="p-4">
										{item.tipo === "producto" ? (
											<button
												...
												data-tooltip-id="tooltip-producto"
											>
												<img
													src={`https://lh3.googleusercontent.com/d/${item.imagen}`}
									...
									<td className="px-6 py-4 text-pink-400 font-semibold dark:text-white">
			...
			<div
				className="grid justify-items-end justify-end items-center mb-8 w-[80%] mx-auto dark:text-white"
				data-aos="fade-up"
			>
				...
				<div className="mt-8">
					...
					>
						<svg
							...
							fill="currentColor"
							className="w-5 h-5 bi bi-cart-check inline-block mr-2"
							viewBox="0 0 16 16"
						>
